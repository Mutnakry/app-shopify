...
exports.Createproduct = (req, res) => {
  const { names,expice,category_id,qty,original_price,sale_price,profit,discount,status,userNote } = req.body;
  const image = req.file ? req.file.filename : null;

  const query = 'INSERT INTO product (names,expice,category_id,qty,original_price,sale_price,profit,discount,status,image,userNote) VALUES (?,?,?,?,?,?,?,?,?,?,?)';
  db.query(query, [names,expice,category_id,qty,original_price,sale_price,profit,discount,status,image,userNote], (err, result) => {
    if (err) {
...
exports.Updateproduct = (req, res) => {
  ...
  db.query('SELECT image FROM categories WHERE id = ?', [id], (err, result) => {
      ...
      if (newImage) {
          ...
          db.query('UPDATE product SET names = ?,expice=?,category_id=?,qty=?,original_price=?,sale_price=?,profit=?,discount=?,status=?,image=?,userUpdate=? WHERE id = ?', [names,expice,category_id,qty,original_price,sale_price,profit,discount,status,newImage,userNote, id], (err, result) => {
              if (err) {
          ...
          db.query('UPDATE UPDATE product SET names = ?,expice=?,category_id=?,qty=?,original_price=?,sale_price=?,profit=?,discount=?,status=?,image=?,userUpdate=? WHERE id = ?', [names,expice,category_id,qty,original_price,sale_price,profit,discount,status,oldImage,userNote, id], (err, result) => {
              if (err) {
                  return res.status(500).send('Error updating category');
              }
              res.json(result);
